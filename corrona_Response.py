...
def mcid(data_base, data):
    ...
    if 10.1<= cdai<22 and improvement>6: response='responder'
    if cdai >22 and improvement>12: response = 'responder'
...
def corrona():
    ...
    path = '/Users/asherameli/Desktop/RA_ClinialCovariates.csv'
    df = pd.read_csv(path, header=None,
                       names=['SUBJECT','Row.names','Sample.ID','Suffix','Specimen.Name','Instrument','PDF','X28S.18S',
...
header = ('SUBJECT','Row.names','Sample.ID','Suffix','Specimen.Name','Instrument','PDF','X28S.18S',
      'RIN','Qubit.ug.ul','Total..ug.','DETD','STUDY.ID','SHIPMENT.NUMBER',	'SAMPLE.ID',
      'CONTAINER',	'ACCESSION.NUMBER',	'DRAW.DATE','DRAW.TIME','SITE...',	'INITIALS',
      'DATE.OF.BIRTH','GENDER',	'VISIT','QC','Scipher_id',	'grp','visit','visitdate','CDate',
      'age','gender','duration_ra','race_grp','race_hispanic','hx_fib','hx_CVD','hx_ser_inf',
    ...
    f.write("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t"
        ...
        "%s\t\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" %
            ...
            data['DATE.OF.BIRTH'],data['GENDER'],data['VISIT'],data['QC'],data['Scipher_id'],data['grp'],data['visit'],
             data['visitdate'],data['CDate'],data['age'],data['gender'],data['duration_ra'],data['race_grp'],data['race_hispanic'],
